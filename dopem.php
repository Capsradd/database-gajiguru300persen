...
if ($is_logged_in && $conn) {
    if (isset($_POST['save_dopem'])) {
        $nim = trim($_POST['nim'] ?? '');
        $nid = trim($_POST['nid'] ?? '');

        $stmt = $conn->prepare('INSERT INTO tbl_dopem (nim, nid) VALUES (?, ?)');

        if ($stmt) {
            $stmt->bind_param('ss', $nim, $nid);
            $stmt->execute();
            $stmt->close();
            header('Location: dopem.php?status=added');
            exit;
        }
    ...
    if (isset($_POST['update_dopem'])) {
        $nim = trim($_POST['nim'] ?? '');
        $nid = trim($_POST['nid'] ?? '');

        $stmt = $conn->prepare('UPDATE tbl_dopem SET nid = ? WHERE nim = ?');

        if ($stmt) {
            $stmt->bind_param('ss', $nid, $nim);
            $stmt->execute();
            $stmt->close();
            header('Location: dopem.php?status=updated');
            exit;
        }
    ...
    if (isset($_POST['delete_dopem'])) {
        $nim = trim($_POST['nim'] ?? '');

        $stmt = $conn->prepare('DELETE FROM tbl_dopem WHERE nim = ?');

        if ($stmt) {
            $stmt->bind_param('s', $nim);
            $stmt->execute();
            $stmt->close();
            header('Location: dopem.php?status=deleted');
            exit;
        }
    ...
    if ($edit_nim !== '') {
        $stmt = $conn->prepare('SELECT nim, nid FROM tbl_dopem WHERE nim = ? LIMIT 1');
        if ($stmt) {
            $stmt->bind_param('s', $edit_nim);
            $stmt->execute();
            $result = $stmt->get_result();
            $edit_dopem = $result ? $result->fetch_assoc() : null;
            $stmt->close();
            $open_modal = 'edit';
        }
    }
    ...
    if ($delete_nim !== '') {
        ...
        if ($stmt) {
            ...
            $stmt->execute();
            $result = $stmt->get_result();
            $delete_dopem = $result ? $result->fetch_assoc() : null;
            $stmt->close();
            $open_modal = 'delete';
        }
    }

    $query = mysqli_query($conn, "SELECT * FROM tbl_dopem ORDER BY nim ASC");

    if ($query) {
        $total_dopem = mysqli_num_rows($query);
    } else {
...
<body class="bg-white text-gray-800 font-sans antialiased flex h-screen overflow-hidden">
    ...
    <main class="flex-1 flex flex-col h-full bg-white overflow-hidden">
        <header class="h-16 flex items-center justify-between px-8 border-b border-gray-200 flex-shrink-0 bg-white">
            <div>
                <h1 class="text-lg font-semibold text-gray-900">Data Dopem</h1>
        ...
        <div class="flex-1 flex flex-col p-8 overflow-hidden min-h-0">
            ...
            <div class="mb-4 rounded-lg border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-700">
                Data dopem berhasil ditambahkan.
            </div>
            <?php elseif ($status === 'updated'): ?>
            <div class="mb-4 rounded-lg border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-700">
                Data dopem berhasil diperbarui.
            </div>
            <?php elseif ($status === 'deleted'): ?>
            <div class="mb-4 rounded-lg border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-700">
                Data dopem berhasil dihapus.
            </div>
            ...
            <div class="flex items-center justify-between mb-8">
                <div class="flex items-center gap-3">
                    <button class="flex items-center gap-2 px-3 py-1.5 text-sm border border-gray-200 rounded-md bg-white hover:bg-gray-50 text-gray-700 font-medium">
                        <i class="fa-regular fa-building"></i> Mahasiswa
                    ...
                    <button class="flex items-center gap-2 px-3 py-1.5 text-sm border border-gray-200 rounded-md bg-white hover:bg-gray-50 text-gray-700 font-medium whitespace-nowrap">
                        <i class="fa-solid fa-filter text-gray-400"></i> Semua Dopem <i class="fa-solid fa-chevron-down text-xs ml-1 text-gray-400"></i>
                    </button>
                </div>
                <div class="flex items-center gap-3">
                    ...
                    <button type="button" onclick="openAddModal()" class="flex items-center gap-2 px-4 py-1.5 text-sm bg-black text-white rounded-md hover:bg-gray-800 font-medium whitespace-nowrap">
                        <i class="fa-solid fa-plus"></i> Tambah Dopem
                    </button>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                <div class="border border-gray-200 rounded-lg p-5 bg-white">
                    ...
                    <div class="text-xs text-gray-400 mb-1">Data pada tabel tbl_dopem</div>
                    <div class="flex items-end justify-between">
                        <span class="text-2xl font-bold"><?php echo number_format($total_dopem); ?></span>
                ...
                <div class="border border-gray-200 rounded-lg p-5 bg-white">
                    ...
                    <div class="text-xs text-gray-400 mb-1">Tambah atau ubah data dopem</div>
                    <div class="flex items-end justify-between">
            ...
            <div class="flex-1 flex flex-col min-h-0">
                <div class="flex items-center justify-between mb-4">
                    <div>
                        <h2 class="text-base font-semibold text-gray-900">Daftar Dopem</h2>
                        <p class="text-xs text-gray-500">Data dosen pembimbing dari database</p>
                    </div>
                    <div class="flex gap-2 border border-gray-200 rounded-md overflow-hidden bg-white">
                        <button type="button" onclick="openExportModal({table: 'tbl_dopem', cols: 'nim,nid', filename: 'dopem_list'})" class="px-3 py-1.5 text-sm font-medium text-gray-700 hover:bg-gray-50 flex items-center gap-2 border-r border-gray-200">
                ...
                <?php if ($query_error): ?>
                <div class="mb-4 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
                    Query error: <?php echo htmlspecialchars($query_error); ?>
                </div>
                <?php endif; ?>
                                ...
                                <tr class="hover:bg-gray-50 searchable-row">
                                    ...
                                    <td class="py-3 px-4 text-right">
                                        <div class="inline-flex items-center gap-2">
                                            <button type="button" data-nim="<?php echo htmlspecialchars($row['nim'], ENT_QUOTES); ?>" data-nid="<?php echo htmlspecialchars($row['nid'], ENT_QUOTES); ?>" onclick="openEditModalFromButton(this)" class="px-3 py-1.5 rounded-md text-xs font-medium bg-blue-50 text-blue-700 hover:bg-blue-100 border border-blue-100">
                                                Edit
                                            </button>
                                            <button type="button" data-nim="<?php echo htmlspecialchars($row['nim'], ENT_QUOTES); ?>" data-nid="<?php echo htmlspecialchars($row['nid'], ENT_QUOTES); ?>" onclick="openDeleteModalFromButton(this)" class="px-3 py-1.5 rounded-md text-xs font-medium bg-red-50 text-red-700 hover:bg-red-100 border border-red-100">
                                                Hapus
                                            </button>
                                        </div>
                                    </td>
    ...
    <div id="addModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-gray-900/60 backdrop-blur-sm px-4">
        <div class="w-full max-w-lg rounded-2xl bg-white shadow-2xl border border-gray-100 overflow-hidden">
            <div class="flex items-center justify-between border-b border-gray-200 px-6 py-4">
                <div>
                    <h3 class="text-lg font-semibold text-gray-900">Tambah Dopem</h3>
                    <p class="text-xs text-gray-500 mt-0.5">Buat data dosen pembimbing baru tanpa pindah halaman.</p>
                </div>
            ...
            <form method="POST" class="p-6 space-y-4">
                ...
                <div class="flex items-center justify-end gap-3 pt-2">
                    ...
                    <button type="submit" name="save_dopem" class="rounded-lg bg-black px-4 py-2 text-sm font-medium text-white hover:bg-gray-800">Simpan</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="editModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-gray-900/60 backdrop-blur-sm px-4">
        <div class="w-full max-w-lg rounded-2xl bg-white shadow-2xl border border-gray-100 overflow-hidden">
            <div class="flex items-center justify-between border-b border-gray-200 px-6 py-4">
                <div>
                    <h3 class="text-lg font-semibold text-gray-900">Edit Dopem</h3>
                    <p class="text-xs text-gray-500 mt-0.5">Perbarui data dosen pembimbing di tempat yang sama.</p>
                </div>
            ...
            <form method="POST" class="p-6 space-y-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">NID</label>
                    <input type="text" id="edit_nid" name="nid" required class="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm focus:border-black focus:outline-none focus:ring-2 focus:ring-black/10">
                </div>
                <div class="flex items-center justify-end gap-3 pt-2">
                    ...
                    <button type="submit" name="update_dopem" class="rounded-lg bg-black px-4 py-2 text-sm font-medium text-white hover:bg-gray-800">Update</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="deleteModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-gray-900/60 backdrop-blur-sm px-4">
        <div class="w-full max-w-lg rounded-2xl bg-white shadow-2xl border border-gray-100 overflow-hidden">
            <div class="flex items-center justify-between border-b border-gray-200 px-6 py-4">
                <div>
                    <h3 class="text-lg font-semibold text-gray-900">Hapus Dopem</h3>
            ...
            <form method="POST" class="p-6 space-y-5">
                ...
                <div class="rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-700">
                    Apakah kamu yakin ingin menghapus data dopem <span id="delete_nid" class="font-semibold"></span>?
                </div>
                <div class="flex items-center justify-end gap-3 pt-2">
                    ...
                    <button type="submit" name="delete_dopem" class="rounded-lg bg-red-600 px-4 py-2 text-sm font-medium text-white hover:bg-red-700">Hapus</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        const editDopem = <?php echo json_encode($edit_dopem); ?>;
        const deleteDopem = <?php echo json_encode($delete_dopem); ?>;
        ...
        function openDeleteModalFromButton(button) {
            ...
            document.getElementById('delete_nid').textContent = button.dataset.nid || '';
        ...
        if (openModalId === 'edit') {
            if (editDopem) {
                document.getElementById('edit_nim').value = editDopem.nim || '';
                document.getElementById('edit_nid').value = editDopem.nid || '';
            }
            showModal('editModal');
        }
        ...
        if (openModalId === 'delete') {
            if (deleteDopem) {
                document.getElementById('delete_nim').value = deleteDopem.nim || '';
                document.getElementById('delete_nid').textContent = deleteDopem.nid || '';
            }
            showModal('deleteModal');
        }
