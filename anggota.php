...
$query = null;
$total_anggota = 0;
$anggota_table_exists = true;
$query_error = '';
$edit_anggota = null;
$delete_anggota = null;

if ($is_logged_in && $conn) {
    $table_check = mysqli_query($conn, "SHOW TABLES LIKE 'anggota'");
    ...
    if ($table_check && mysqli_num_rows($table_check) > 0) {
        if (isset($_POST['save_anggota'])) {
            ...
            $stmt = $conn->prepare('INSERT INTO anggota (nim, nama_anggota, tugas) VALUES (?, ?, ?)');

            if ($stmt) {
                $stmt->bind_param('sss', $nim, $nama_anggota, $tugas);
        ...
        if (isset($_POST['update_anggota'])) {
            ...
            $stmt = $conn->prepare('UPDATE anggota SET nama_anggota = ?, tugas = ? WHERE nim = ?');

            if ($stmt) {
                $stmt->bind_param('sss', $nama_anggota, $tugas, $nim);

                if ($stmt->execute()) {
                    $stmt->close();
                    header('Location: anggota.php?status=updated');
                    exit;
                }
        ...
        if (isset($_POST['delete_anggota'])) {
            ...
            $stmt = $conn->prepare('DELETE FROM anggota WHERE nim = ?');
        ...
        if ($edit_nim !== '') {
            $stmt = $conn->prepare('SELECT nim, nama_anggota, tugas FROM anggota WHERE nim = ? LIMIT 1');
            if ($stmt) {
                $stmt->bind_param('s', $edit_nim);
                $stmt->execute();
                $result = $stmt->get_result();
                $edit_anggota = $result ? $result->fetch_assoc() : null;
                $stmt->close();
                $open_modal = 'edit';
            }
        }
        ...
        if ($delete_nim !== '') {
            ...
            if ($stmt) {
                ...
                $stmt->execute();
                $result = $stmt->get_result();
                $delete_anggota = $result ? $result->fetch_assoc() : null;
                $stmt->close();
                $open_modal = 'delete';
            }
        }
...
<head>
    ...
    <title>Data Anggota - Unidb</title>
...
<body class="bg-white text-gray-800 font-sans antialiased flex h-screen overflow-hidden">
    ...
    <main class="flex-1 flex flex-col h-full bg-white overflow-hidden">
        <header class="h-16 flex items-center justify-between px-8 border-b border-gray-200 flex-shrink-0 bg-white">
            <div>
                <h1 class="text-lg font-semibold text-gray-900">Data Anggota Kelompok</h1>
            ...
            <div class="flex items-center gap-4">
                <div class="relative">
                    ...
                    <input type="text" placeholder="Search Anggota" class="bg-gray-50 text-sm rounded-md pl-9 pr-4 py-2 w-64 focus:outline-none border border-gray-200">
                </div>
        ...
        <div class="flex-1 overflow-auto p-8">
            ...
            <?php if ($is_logged_in && !$anggota_table_exists): ?>
            <div class="mb-6 rounded-lg border border-amber-200 bg-amber-50 p-4 text-amber-800">
                <div class="flex items-start justify-between gap-4 flex-wrap">
                    <div>
                        <p class="text-sm font-semibold">Tabel anggota belum ditemukan.</p>
                        <p class="text-sm mt-1">Buka SQL Console untuk membuat tabel terlebih dahulu.</p>
                    </div>
                    <a href="console.php" class="inline-flex items-center gap-2 rounded-md bg-amber-600 px-3 py-2 text-sm font-medium text-white hover:bg-amber-700">
                        <i class="fa-solid fa-terminal"></i> Buka SQL Console
                    </a>
                </div>
                <div class="mt-3 rounded-md border border-amber-200 bg-white p-3">
                    ...
                    <pre class="text-xs leading-5 overflow-auto text-gray-700">CREATE TABLE anggota (
    ...
    tugas VARCHAR(255) NOT NULL
            ...
            <div class="mb-6 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
                Gagal membaca data anggota: <?php echo htmlspecialchars($query_error); ?>
            </div>
            <?php endif; ?>
            ...
            <div class="flex items-center justify-between mb-8">
                <div class="flex items-center gap-3">
                    <button class="flex items-center gap-2 px-3 py-1.5 text-sm border border-gray-200 rounded-md bg-white hover:bg-gray-50 text-gray-700 font-medium">
                        <i class="fa-regular fa-folder-open"></i> Kelompok
                    ...
                    <button class="flex items-center gap-2 px-3 py-1.5 text-sm border border-gray-200 rounded-md bg-white hover:bg-gray-50 text-gray-700 font-medium whitespace-nowrap">
                        <i class="fa-solid fa-filter text-gray-400"></i> Semua Anggota <i class="fa-solid fa-chevron-down text-xs ml-1 text-gray-400"></i>
                    </button>
                </div>
                <div class="flex items-center gap-3">
                    ...
                    <button type="button" onclick="openAddModal()" class="flex items-center gap-2 px-4 py-1.5 text-sm bg-black text-white rounded-md hover:bg-gray-800 font-medium whitespace-nowrap <?php echo !$anggota_table_exists ? 'opacity-60 cursor-not-allowed' : ''; ?>" <?php echo !$anggota_table_exists ? 'disabled' : ''; ?>>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                <div class="border border-gray-200 rounded-lg p-5 bg-white">
                    ...
                    <div class="text-xs text-gray-400 mb-1">Data pada tabel anggota</div>
                    <div class="flex items-end justify-between">
                        <span class="text-2xl font-bold"><?php echo number_format($total_anggota); ?></span>
                ...
                <div class="border border-gray-200 rounded-lg p-5 bg-white">
                    <div class="flex justify-between items-start mb-4">
                        <span class="text-sm font-medium text-gray-600">Pembagian Tugas</span>
                ...
                <div class="border border-gray-200 rounded-lg p-5 bg-white">
                    ...
                    <div class="flex items-end justify-between">
                        <span class="text-2xl font-bold"><?php echo $is_logged_in ? 'Online' : 'Offline'; ?></span>
                        <span class="text-xs font-semibold px-1.5 py-0.5 rounded <?php echo $is_logged_in ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600'; ?>">
                            <?php echo $is_logged_in ? 'Connected' : 'Disconnected'; ?>
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div class="mb-4">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <div class="flex gap-2 border border-gray-200 rounded-md overflow-hidden bg-white">
                        ...
                        <button type="button" onclick="openExportModal({table: 'anggota', cols: 'nim,nama_anggota,tugas', filename: 'anggota_list'})" class="px-3 py-1.5 text-sm font-medium text-gray-700 hover:bg-gray-50 flex items-center gap-2 border-r border-gray-200">
                            <i class="fa-solid fa-file-export text-gray-400"></i> Export
                        </button>
                        <?php else: ?>
                        <a href="console.php" class="px-3 py-1.5 text-sm font-medium text-amber-700 hover:bg-amber-50 flex items-center gap-2 border-r border-gray-200">
                            <i class="fa-solid fa-terminal text-amber-500"></i> Buat Tabel
                        </a>
                        <?php endif; ?>
                    </div>
                </div>
                    ...
                    <table class="w-full text-left border-collapse text-sm">
                        <thead>
                            <tr class="border-b border-gray-200 bg-gray-50/50">
                                ...
                                <th class="font-medium text-gray-500 py-3 px-4">Nama Anggota</th>
                                <th class="font-medium text-gray-500 py-3 px-4">Bagian Tugas</th>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            <?php if ($query && mysqli_num_rows($query) > 0): ?>
                                    ...
                                    <td class="py-3 px-4">
                                        <div class="flex items-center gap-3">
                                            <div class="w-6 h-6 rounded-full bg-gray-200 flex items-center justify-center text-[10px] font-bold text-gray-600 uppercase">
                                                <?php echo isset($row['nama_anggota'][0]) ? htmlspecialchars($row['nama_anggota'][0]) : '?'; ?>
                                            </div>
                                            <span class="font-medium text-gray-900"><?php echo htmlspecialchars($row['nama_anggota']); ?></span>
                                        </div>
                                    </td>
                                    <td class="py-3 px-4 text-gray-600"><?php echo htmlspecialchars($row['tugas']); ?></td>
                                    <td class="py-3 px-4 text-right">
                                        <div class="inline-flex items-center gap-2">
                                            <button type="button" data-nim="<?php echo htmlspecialchars($row['nim'], ENT_QUOTES); ?>" data-nama-anggota="<?php echo htmlspecialchars($row['nama_anggota'], ENT_QUOTES); ?>" data-tugas="<?php echo htmlspecialchars($row['tugas'], ENT_QUOTES); ?>" onclick="openEditModalFromButton(this)" class="px-3 py-1.5 rounded-md text-xs font-medium bg-blue-50 text-blue-700 hover:bg-blue-100 border border-blue-100">
                                                Edit
                                            </button>
                                            <button type="button" data-nim="<?php echo htmlspecialchars($row['nim'], ENT_QUOTES); ?>" data-nama-anggota="<?php echo htmlspecialchars($row['nama_anggota'], ENT_QUOTES); ?>" data-tugas="<?php echo htmlspecialchars($row['tugas'], ENT_QUOTES); ?>" onclick="openDeleteModalFromButton(this)" class="px-3 py-1.5 rounded-md text-xs font-medium bg-red-50 text-red-700 hover:bg-red-100 border border-red-100">
                                                Hapus
                                            </button>
                                        </div>
                                    </td>
                                ...
                                <?php endwhile; ?>
                            <?php elseif ($anggota_table_exists): ?>
                                <tr>
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="5" class="py-12 text-center text-amber-700 bg-amber-50/40">
                                        <i class="fa-solid fa-triangle-exclamation text-3xl mb-3 text-amber-500"></i>
                                        <p class="text-sm font-semibold">Tabel anggota belum ada.</p>
                                        <a href="console.php" class="inline-flex items-center gap-2 mt-3 rounded-md bg-amber-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-amber-700">
                                            <i class="fa-solid fa-terminal"></i> Buka SQL Console
                                        </a>
                                    </td>
                                </tr>
    ...
    <div id="addModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-gray-900/60 backdrop-blur-sm px-4">
        <div class="w-full max-w-lg rounded-2xl bg-white shadow-2xl border border-gray-100 overflow-hidden">
            <div class="flex items-center justify-between border-b border-gray-200 px-6 py-4">
                <div>
                    <h3 class="text-lg font-semibold text-gray-900">Tambah Anggota</h3>
                    <p class="text-xs text-gray-500 mt-0.5">Tambahkan data anggota baru tanpa pindah halaman.</p>
                </div>
            ...
            <form method="POST" class="p-6 space-y-4">
                <div>
                    ...
                    <input type="text" name="nim" required class="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm focus:border-black focus:outline-none focus:ring-2 focus:ring-black/10" placeholder="Contoh: 220101001">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Nama Anggota</label>
                    <input type="text" name="nama_anggota" required class="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm focus:border-black focus:outline-none focus:ring-2 focus:ring-black/10" placeholder="Contoh: Budi Santoso">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Bagian Tugas</label>
                    <input type="text" name="tugas" required class="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm focus:border-black focus:outline-none focus:ring-2 focus:ring-black/10" placeholder="Contoh: Frontend & Dokumentasi">
                </div>
                <div class="flex items-center justify-end gap-3 pt-2">
                    ...
                    <button type="submit" name="save_anggota" class="rounded-lg bg-black px-4 py-2 text-sm font-medium text-white hover:bg-gray-800">Simpan</button>
                </div>
            </form>
        </div>
    </div>
        ...
        <div class="w-full max-w-lg rounded-2xl bg-white shadow-2xl border border-gray-100 overflow-hidden">
            <div class="flex items-center justify-between border-b border-gray-200 px-6 py-4">
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-0.5">Perbarui data anggota tanpa pindah halaman.</p>
                </div>
            ...
            <form method="POST" class="p-6 space-y-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Nama Anggota</label>
                    <input type="text" id="edit_nama_anggota" name="nama_anggota" required class="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm focus:border-black focus:outline-none focus:ring-2 focus:ring-black/10">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Bagian Tugas</label>
                    <input type="text" id="edit_tugas" name="tugas" required class="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm focus:border-black focus:outline-none focus:ring-2 focus:ring-black/10">
                </div>
                <div class="flex items-center justify-end gap-3 pt-2">
                    ...
                    <button type="submit" name="update_anggota" class="rounded-lg bg-black px-4 py-2 text-sm font-medium text-white hover:bg-gray-800">Update</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="deleteModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-gray-900/60 backdrop-blur-sm px-4">
        <div class="w-full max-w-lg rounded-2xl bg-white shadow-2xl border border-gray-100 overflow-hidden">
            <div class="flex items-center justify-between border-b border-gray-200 px-6 py-4">
                <div>
                    <h3 class="text-lg font-semibold text-gray-900">Hapus Anggota</h3>
            ...
            <form method="POST" class="p-6 space-y-5">
                ...
                <div class="rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-700">
                    Apakah kamu yakin ingin menghapus data anggota <span id="delete_nama_anggota" class="font-semibold"></span>?
                </div>
                <div class="flex items-center justify-end gap-3 pt-2">
                    ...
                    <button type="submit" name="delete_anggota" class="rounded-lg bg-red-600 px-4 py-2 text-sm font-medium text-white hover:bg-red-700">Hapus</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        const editAnggota = <?php echo json_encode($edit_anggota); ?>;
        const deleteAnggota = <?php echo json_encode($delete_anggota); ?>;
        ...
        function openEditModalFromButton(button) {
            ...
            document.getElementById('edit_nama_anggota').value = button.dataset.namaAnggota || '';
        ...
        function openDeleteModalFromButton(button) {
            ...
            document.getElementById('delete_nama_anggota').textContent = button.dataset.namaAnggota || '';
        ...
        if (openModalId === 'edit') {
            if (editAnggota) {
                document.getElementById('edit_nim').value = editAnggota.nim || '';
                document.getElementById('edit_nama_anggota').value = editAnggota.nama_anggota || '';
                document.getElementById('edit_tugas').value = editAnggota.tugas || '';
            }
            showModal('editModal');
        }
        ...
        if (openModalId === 'delete') {
            if (deleteAnggota) {
                document.getElementById('delete_nim').value = deleteAnggota.nim || '';
                document.getElementById('delete_nama_anggota').textContent = deleteAnggota.nama_anggota || '';
            }
            showModal('deleteModal');
        }
