...
if ($is_logged_in && $conn) {
    if (isset($_POST['save_mhs'])) {
        $nim     = trim($_POST['nim'] ?? '');
        $namamhs = trim($_POST['namamhs'] ?? '');
        $phone   = trim($_POST['phone'] ?? '');

        $stmt = $conn->prepare('INSERT INTO tbl_mhs (nim, namamhs, phone) VALUES (?, ?, ?)');
        if ($stmt) {
            $stmt->bind_param('sss', $nim, $namamhs, $phone);
            $stmt->execute();
            $stmt->close();
            header('Location: mahasiswa.php?status=added');
            exit;
        }
        $status = 'error';
    }
    ...
    if (isset($_POST['update_mhs'])) {
        ...
        $stmt = $conn->prepare('UPDATE tbl_mhs SET namamhs = ?, phone = ? WHERE nim = ?');
        if ($stmt) {
            $stmt->bind_param('sss', $namamhs, $phone, $nim);
            $stmt->execute();
            $stmt->close();
            header('Location: mahasiswa.php?status=updated');
            exit;
        }
        $status = 'error';
    }
    ...
    if (isset($_POST['delete_mhs'])) {
        ...
        $stmt = $conn->prepare('DELETE FROM tbl_mhs WHERE nim = ?');
        if ($stmt) {
    ...
    if ($edit_nim !== '') {
        $stmt = $conn->prepare('SELECT nim, namamhs, phone FROM tbl_mhs WHERE nim = ? LIMIT 1');
        if ($stmt) {
            $stmt->bind_param('s', $edit_nim);
            $stmt->execute();
            $result = $stmt->get_result();
            $edit_mhs = $result ? $result->fetch_assoc() : null;
            $stmt->close();
            $open_modal = 'edit';
        }
    }
    ...
    if ($delete_nim !== '') {
        ...
        if ($stmt) {
            ...
            $stmt->execute();
            $result = $stmt->get_result();
            $delete_mhs = $result ? $result->fetch_assoc() : null;
            $stmt->close();
            $open_modal = 'delete';
        }
    }
...
if ($is_logged_in && $conn) {
    $query = mysqli_query($conn, 'SELECT * FROM tbl_mhs ORDER BY nim ASC');
    if ($query) {
        $total_mhs = mysqli_num_rows($query);
    }
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Data Mahasiswa - Unidb</title>
...
<body class="bg-white text-gray-800 font-sans antialiased flex h-screen overflow-hidden">
    ...
    <main class="flex-1 flex flex-col h-full bg-white overflow-hidden">
        <header class="h-16 flex items-center justify-between px-8 border-b border-gray-200 flex-shrink-0 bg-white">
            <div>
                <h1 class="text-lg font-semibold text-gray-900">Data Mahasiswa</h1>
            ...
            <div class="flex items-center gap-4">
                <div class="relative">
                    ...
                    <input type="text" id="searchInput" placeholder="Cari Mahasiswa" class="bg-gray-50 text-sm rounded-md pl-9 pr-4 py-2 w-64 focus:outline-none border border-gray-200">
                </div>
                <button class="text-gray-400 hover:text-gray-600 w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-100 border border-transparent">
        ...
        <div class="flex-1 flex flex-col p-8 overflow-hidden min-h-0">
            ...
            <div class="flex items-center justify-between mb-8">
                <div class="flex items-center gap-3">
                    ...
                    <button class="flex items-center gap-2 px-3 py-1.5 text-sm border border-gray-200 rounded-md bg-white hover:bg-gray-50 text-gray-700 font-medium whitespace-nowrap">
                        <i class="fa-solid fa-filter text-gray-400"></i> Semua Mahasiswa
                        <i class="fa-solid fa-chevron-down text-xs ml-1 text-gray-400"></i>
                    </button>
                </div>
                <div class="flex items-center gap-3">
                    <button onclick="window.location.href='mahasiswa.php'" class="flex items-center gap-2 px-3 py-1.5 text-sm border border-gray-200 rounded-md bg-white hover:bg-gray-50 text-gray-700 font-medium whitespace-nowrap">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                <div class="border border-gray-200 rounded-lg p-5 bg-white">
                    ...
                    <div class="text-xs text-gray-400 mb-1">Data pada tabel tbl_mhs</div>
                    <div class="flex items-end justify-between">
                        <span class="text-2xl font-bold"><?php echo number_format($total_mhs); ?></span>
                ...
                <div class="border border-gray-200 rounded-lg p-5 bg-white">
                    ...
                    <div class="text-xs text-gray-400 mb-1">Tambah atau ubah data mahasiswa</div>
                    <div class="flex items-end justify-between">
            ...
            <div class="flex-1 flex flex-col min-h-0">
                <div class="flex items-center justify-between mb-4">
                    <div>
                        <h2 class="text-base font-semibold text-gray-900">Daftar Mahasiswa</h2>
                        <p class="text-xs text-gray-500">Data mahasiswa dari database</p>
                    </div>
                    <div class="flex gap-2 border border-gray-200 rounded-md overflow-hidden bg-white">
                        <button type="button" onclick="openExportModal({table: 'tbl_mhs', cols: 'nim,namamhs,phone', filename: 'mahasiswa_list'})" class="px-3 py-1.5 text-sm font-medium text-gray-700 hover:bg-gray-50 flex items-center gap-2 border-r border-gray-200">
                ...
                <div class="flex-1 min-h-0 border border-gray-200 rounded-lg overflow-auto bg-white">
                    <table class="w-full text-left border-collapse text-sm" id="mhsTable">
                        <thead>
                            <tr class="border-b border-gray-200 bg-gray-50/50">
                                ...
                                <th class="font-medium text-gray-500 py-3 px-4">No. HP</th>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            <?php if ($query && mysqli_num_rows($query) > 0): ?>
                                ...
                                <tr class="hover:bg-gray-50 mhs-row">
                                    <td class="py-3 px-4 text-center text-gray-500 row-no"><?php echo $no++; ?></td>
                                    <td class="py-3 px-4 text-gray-500 font-mono text-xs font-nim"><?php echo htmlspecialchars($row['nim']); ?></td>
                                    <td class="py-3 px-4">
                                        <div class="flex items-center gap-3">
                                            <div class="w-6 h-6 rounded-full bg-gray-200 flex items-center justify-center text-[10px] font-bold text-gray-600 uppercase">
                                                <?php echo isset($row['namamhs'][0]) ? htmlspecialchars($row['namamhs'][0]) : '?'; ?>
                                            </div>
                                            <span class="font-medium text-gray-900 font-nama"><?php echo htmlspecialchars($row['namamhs']); ?></span>
                                        </div>
                                    </td>
                                    <td class="py-3 px-4 text-gray-700 font-phone"><?php echo htmlspecialchars($row['phone'] ?? '-'); ?></td>
                                    <td class="py-3 px-4 text-right">
                                        <div class="inline-flex items-center gap-2">
                                            <button type="button"
                                                data-nim="<?php echo htmlspecialchars($row['nim'], ENT_QUOTES); ?>"
                                                data-namamhs="<?php echo htmlspecialchars($row['namamhs'], ENT_QUOTES); ?>"
                                            ...
                                            <button type="button"
                                                ...
                                                class="px-3 py-1.5 rounded-md text-xs font-medium bg-red-50 text-red-700 hover:bg-red-100 border border-red-100">
                                                Hapus
                                            </button>
                                        </div>
                                    </td>
                            ...
                            <tr id="emptyRow">
                                <td colspan="5" class="py-12 text-center text-gray-400">
                                    ...
                                    <p class="text-sm font-medium text-gray-500">Belum ada data mahasiswa.</p>
                                </td>
                            </tr>
                            <?php endif; ?>
        ...
        <div class="w-full max-w-lg rounded-2xl bg-white shadow-2xl border border-gray-100 overflow-hidden">
            <div class="flex items-center justify-between border-b border-gray-200 px-6 py-4">
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-0.5">Buat data mahasiswa baru.</p>
                </div>
    ...
    <div id="editModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-gray-900/60 backdrop-blur-sm px-4">
        <div class="w-full max-w-lg rounded-2xl bg-white shadow-2xl border border-gray-100 overflow-hidden">
            <div class="flex items-center justify-between border-b border-gray-200 px-6 py-4">
                <div>
                    <h3 class="text-lg font-semibold text-gray-900">Edit Mahasiswa</h3>
                    <p class="text-xs text-gray-500 mt-0.5">Perbarui data mahasiswa.</p>
                </div>
            ...
            <form method="POST" class="p-6 space-y-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Nama Mahasiswa</label>
                    <input type="text" id="edit_namamhs" name="namamhs" required class="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm focus:border-black focus:outline-none focus:ring-2 focus:ring-black/10">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">No. HP</label>
                    <input type="text" id="edit_phone" name="phone" class="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm focus:border-black focus:outline-none focus:ring-2 focus:ring-black/10">
                </div>
                <div class="flex items-center justify-end gap-3 pt-2">
                    ...
                    <button type="submit" name="update_mhs" class="rounded-lg bg-black px-4 py-2 text-sm font-medium text-white hover:bg-gray-800">Update</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="deleteModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-gray-900/60 backdrop-blur-sm px-4">
        <div class="w-full max-w-lg rounded-2xl bg-white shadow-2xl border border-gray-100 overflow-hidden">
            <div class="flex items-center justify-between border-b border-gray-200 px-6 py-4">
                <div>
                    <h3 class="text-lg font-semibold text-gray-900">Hapus Mahasiswa</h3>
            ...
            <form method="POST" class="p-6 space-y-5">
                ...
                <div class="rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-700">
                    Apakah kamu yakin ingin menghapus data mahasiswa <span id="delete_namamhs" class="font-semibold"></span>?
                </div>
                <div class="flex items-center justify-end gap-3 pt-2">
                    ...
                    <button type="submit" name="delete_mhs" class="rounded-lg bg-red-600 px-4 py-2 text-sm font-medium text-white hover:bg-red-700">Hapus</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        function closeModal(id) {
            ...
            // Opsional: bersihkan query string URL jika modal berasal dari metode GET PHP (?edit= atau ?delete=)
            const url = new URL(window.location);
            if(url.searchParams.has('edit') || url.searchParams.has('delete')) {
        ...
        function openEditModalFromButton(btn) {
            ...
            document.getElementById('edit_namamhs').value = btn.dataset.namamhs || '';
        ...
        function openDeleteModalFromButton(btn) {
            ...
            document.getElementById('delete_namamhs').textContent = btn.dataset.namamhs || '';
        ...
        // --- Fitur Live Search & Penomoran Otomatis Dinamis ---
        document.getElementById('searchInput').addEventListener('input', function () {
            const q = this.value.toLowerCase().trim();
            const rows = document.querySelectorAll('#mhsTable tbody tr.mhs-row');
            const noResultRow = document.getElementById('noResultRow');
            let visibleCount = 0;

            rows.forEach(row => {
                const nim = row.querySelector('.font-nim').textContent.toLowerCase();
                const nama = row.querySelector('.font-nama').textContent.toLowerCase();
                const phone = row.querySelector('.font-phone').textContent.toLowerCase();

                if (nim.includes(q) || nama.includes(q) || phone.includes(q)) {
                    row.classList.remove('hidden');
                    visibleCount++;
                    // Update nomor baris secara dinamis agar tetap berurutan (1, 2, 3...)
        ...
        // --- Integrasi Fallback Modal PHP PHP Server-Side ke Client JavaScript ---
        document.addEventListener("DOMContentLoaded", function() {
            <?php if ($open_modal === 'edit' && $edit_mhs): ?>
                document.getElementById('edit_nim').value     = <?php echo json_encode($edit_mhs['nim']); ?>;
                document.getElementById('edit_namamhs').value = <?php echo json_encode($edit_mhs['namamhs']); ?>;
                document.getElementById('edit_phone').value   = <?php echo json_encode($edit_mhs['phone'] ?? ''); ?>;
                showModal('editModal');
            <?php elseif ($open_modal === 'delete' && $delete_mhs): ?>
                document.getElementById('delete_nim').value          = <?php echo json_encode($delete_mhs['nim']); ?>;
                document.getElementById('delete_namamhs').textContent = <?php echo json_encode($delete_mhs['namamhs']); ?>;
                showModal('deleteModal');
            <?php endif; ?>
        });
    </script>
